create or replace package body loggerr_test
as
  ...
  begin
    ...
    if loggerr.is_number(p_str => 'a') then
      util_add_error('not failing on letter');
    end if;

    if not loggerr.is_number(p_str => '1') then
  ...
  begin
    ...
    loggerr.append_param(l_params, 'p_test1', 'test1');
    loggerr.append_param(l_params, 'p_test2', 'test2');

    l_clob := loggerr.get_param_clob(p_params => l_params);
  ...
  begin
    ...
    if sys_context(loggerr.g_context_name, 'TEST') != 'test_value' then
    ...
    if sys_context(loggerr.g_context_name, 'TEST') != 'test_client_id' then
  ...
  begin
    ...
    -- Test empty params
    l_clob := 'test';
    l_return := loggerr.set_extra_with_params(
      p_extra => l_clob,
      p_params => l_params);
...
p_test2: test2' then
  ...
  begin
    ...
    l_clob := loggerr.get_sys_context(
      p_detail_level => 'USER',
      p_vertical => false,
      p_show_null => true);
  ...
  begin
    ...
    l_bool := loggerr.admin_security_check;
  ...
  begin
    ...
    l_level := loggerr.get_level_number;

    if l_level != loggerr.g_debug then
    ...
    dbms_session.set_identifier(gc_client_id);
    loggerr.set_level(
      p_level => loggerr.g_error,
      p_client_id => sys_context('userenv','client_identifier')
    );
    l_level := loggerr.get_level_number;

    if l_level != loggerr.g_error then
  ...
  begin
    ...
    if not loggerr.include_call_stack then
    ...
    -- reset contexts so that it looks at new one (could have called loggerr.configure but more than what I need here)
    loggerr.null_global_contexts;

    if loggerr.include_call_stack then
  ...
  begin
    ...
    if loggerr.date_text_format_base (
  ...
  begin
    ...
    loggerr.set_level(p_level => loggerr.g_error);

    l_scope := util_get_unique_scope;
    loggerr.log_internal(
      p_text => 'test',
...
p_test1: test1' then
  ...
  begin
    ...
    if 1=2
      or sys_context(loggerr.g_context_name,'level') is not null
      or sys_context(loggerr.g_context_name,'include_call_stack') is not null
      or sys_context(loggerr.g_context_name,'plugin_fn_error') is not null
      then
  ...
  begin
    ...
    if loggerr.convert_level_char_to_num(p_level => loggerr.g_error_name) != loggerr.g_error then
  ...
  begin
    ...
    if loggerr.convert_level_num_to_char(p_level => loggerr.g_information) != loggerr.g_information_name then
      util_add_error('Not converting properly');
    end if;
  ...
  begin
    ...
    l_temp := loggerr.get_character_codes(
  		p_string =>
'Test
new line',
...
'Common Codes: 13=Line Feed, 10=Carriage Return, 32=Space, 9=Tab
  ...
  begin
    ...
    for i in test_type.first .. test_type.last loop
      ...
      if test_type(i) = 'global' then
        ...
        -- Client
        -- Reset global level
        loggerr.set_level(p_level => loggerr.g_debug);
      ...
      if loggerr.ok_to_log(p_level => loggerr.g_debug) then
        util_add_error('not registering lower levels. Test Type: ' || test_type(i));
      end if;

      -- Should be true
      if not loggerr.ok_to_log(p_level => loggerr.g_error) then
        util_add_error('failing when same level. Test Type: ' || test_type(i));
      end if;

      -- Should be true
      if not loggerr.ok_to_log(p_level => loggerr.g_permanent) then
  ...
  begin
    ...
    -- Should not log
    loggerr.set_level(p_level => loggerr.g_permanent);
  ...
  begin
    ...
    l_log_fns(l_log_fns.count + 1) := get_log_fn('log_permanent', loggerr.g_off, loggerr.g_permanent, loggerr.g_debug);
    l_log_fns(l_log_fns.count + 1) := get_log_fn('log_warning', loggerr.g_error, loggerr.g_warning, loggerr.g_debug);
    l_log_fns(l_log_fns.count + 1) := get_log_fn('log_information', loggerr.g_warning, loggerr.g_information, loggerr.g_debug);
    l_log_fns(l_log_fns.count + 1) := get_log_fn('log', loggerr.g_warning, loggerr.g_debug, loggerr.g_debug);
    ...
    for i in l_log_fns.first .. l_log_fns.last loop
      ...
      for x in (
        ...
        if x.action = 'off' then
          -- Test off
          loggerr.set_level(l_log_fns(i).level_off);
        elsif x.action = 'self' then
          -- Test self
          loggerr.set_level(l_log_fns(i).level_self);
        elsif x.action = 'on' then
          -- Test on
          loggerr.set_level(l_log_fns(i).level_on);
        end if;

        l_scope := util_get_unique_scope;
        l_sql := 'begin loggerr.' || l_log_fns(i).fn_name || q'!('test', :scope); end;!';
  ...
  begin
    ...
    loggerr.set_level(loggerr.g_timing);
  ...
  begin
    ...
    loggerr.set_level(loggerr.g_debug); -- Time stop only requires g_debug
  ...
  begin
    ...
    loggerr.set_level(loggerr.g_debug);
    ...
    l_text := loggerr.time_stop(p_unit => l_unit_name);
  ...
  begin
    ...
    l_text := loggerr.time_stop_seconds(p_unit => l_unit_name);
  ...
  begin
    ...
    l_pref := nvl(loggerr.get_pref('LEVEL'), 'a');
    if l_pref != loggerr.g_debug_name then
      util_add_error('Global level not fetching correctly');
    end if;

    dbms_session.set_identifier(gc_client_id);
    loggerr.set_level(
      p_level => loggerr.g_warning,
      p_client_id => gc_client_id);
    l_pref := nvl(loggerr.get_pref('LEVEL'), 'a');
    if l_pref != loggerr.g_warning_name then
  ...
  begin
    ...
    loggerr.unset_client_level(p_client_id => gc_client_id);
    log_and_count;
    if l_count != 0 then
  ...
  begin
    ...
    if loggerr.tochar(1) != '1' then
      util_add_error('number');
    end if;

    l_val := loggerr.tochar(to_date('1-Jan-2013'));
    if l_val != '01-JAN-2013 00:00:00' then
      util_add_error('date: ' || l_val);
    end if;

    l_val := loggerr.tochar(to_timestamp ('10-sep-02 14:10:10.123000', 'dd-mon-rr hh24:mi:ss.ff'));
    if l_val != '10-SEP-2002 14:10:10:123000000' then
      util_add_error('timestamp: ' || l_val);
    end if;

    l_val := loggerr.tochar(to_timestamp_tz('1999-12-01 11:00:00 -8:00', 'yyyy-mm-dd hh:mi:ss tzh:tzm'));
    ...
    l_val := loggerr.tochar(true) || ':' || loggerr.tochar(false);
